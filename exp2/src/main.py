...
def solve(account):
    ...
    for day in days:
        ...
        for vari,deli in changed_oi.keys():
            if cursor.execute("select settle from market.contract_daily where vari=%s and deli=%s and day=%s limit 1", (vari,deli,day))==0:
                continue
            settle = cursor.fetchone()[0]
            if cursor.execute("select settle from market.contract_daily where vari=%s and deli=%s and day=%s limit 1", (vari,deli,lastday))==0:
                continue
    ...
    try:
        ...
        result = ols.fit()
        cursor.execute("insert into traderType.exp2 (account,samples,coef,p) values (%s,%s,%s,%s)", 
